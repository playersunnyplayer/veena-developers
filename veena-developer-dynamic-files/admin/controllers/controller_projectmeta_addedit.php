<?
class _projectmeta_addedit extends controller
{
	...
	function onload()
	{
		...
		$this->app->assign("to_do","Project Meta");
	}
	
	function load_data()
	{
		$id=$this->app->getGetVar('id');
		$obj_model_product = $this->app->load_model("projectmeta", $id);
	...
	$id=$this->app->getPostVar('id');		
		...
		if($id!="")
		{
			...
			$obj_model_record = $this->app->load_model("projectmeta");
			...
			{
				@unlink('../'.$this->app->get_user_config("projectmeta").'/'.$result[0]["image"]);
				@unlink('../'.$this->app->get_user_config("projectmeta").'/mediumthumb'.$result[0]["image"]);
				@unlink('../'.$this->app->get_user_config("projectmeta").'/thumb'.$result[0]["image"]);
				$update_field['image']='';									
			}	
			...
			if(!empty($_FILES['image']['name']))
			{
				...
				$image=$this->app->utility->resize_single_image($_FILES['image']['name'],$_FILES['image']['tmp_name'],$this->app->get_user_config("projectmeta"),'412');	
			...
			{	
				...
				$this->app->redirect("index.php?view=projectmeta_list");
			}
			else
			{
		...
		else
		{
			...
			$obj_model_product = $this->app->load_model("projectmeta");
			$obj_model_product->map_fields($update_field);
