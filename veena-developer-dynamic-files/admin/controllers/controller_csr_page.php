<?
	class _csr_page extends controller
	{
		function init()
		{	
		}
		...
		function onload()
		{		
			$obj_model_generel_settings = $this->app->load_model("csr_detail");
		...
		{
			$obj_model_record = $this->app->load_model("csr_detail");
			...
			{
				@unlink('../'.$this->app->get_user_config("csr").'/'.$result[0]["image"]);
				$update_field['image']='';									
			}

			if(!empty($_FILES['about_image']['name']))
			{
				$image=$this->app->utility->resize_single_image($_FILES['about_image']['name'],$_FILES['about_image']['tmp_name'],$this->app->get_user_config("csr"),'550');
				$update_field["image"] = $image;
			}

			if($result[0]["page_image"]!=NULL && $_FILES['career_page_image']['error'] == 0)
			{
				@unlink('../'.$this->app->get_user_config("csr").'/'.$result[0]["page_image"]);
				$update_field['page_image']='';									
			}
			
			if(!empty($_FILES['career_page_image']['name']))
			{
				$career_page_image=$this->app->utility->resize_single_image($_FILES['career_page_image']['name'],$_FILES['career_page_image']['tmp_name'],$this->app->get_user_config("csr"),'1920');
			...
			if($update_id!=NULL)
			{
				$this->app->utility->set_message("CSR Records updated successfully.", "SUCCESS");
				$this->app->redirect("index.php?view=csr_page");
			}
			else
			{
