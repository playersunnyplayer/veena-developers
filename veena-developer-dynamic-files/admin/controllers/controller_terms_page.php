<?
	class _terms_page extends controller
	{
		function init()
		{	
		}
		...
		{		
			...
			$rs = $obj_model_generel_settings->execute("SELECT", false,"","id=2");
		...
		function update_data()
		{
			...
			$result=$obj_model_record->execute("SELECT", false, "", "id=2");
			...
			$update_id = $obj_model_generel_settings->execute("UPDATE", false, "", "id=2");		
			if($update_id!=NULL)
			{
				$this->app->utility->set_message("Terms & Condition updated successfull...", "SUCCESS");
				$this->app->redirect("index.php?view=terms_page");
			}
			else
			{
