...
if($get_actionType=="team_list")
{
	...
	for($i=0;$i<count($result);$i++)
	{
		...
		$folder='team';
			...
			$edit_btn='<a data-toggle="tooltip" data-placement="top" title="Edit" href="index.php?view=team_addedit&id='.$result[$i]['id'].'" class="btn btn-xs btn-primary btn-icon mg-r-5"><i class="fas fa-edit"></i></a>';	
			
			$delete_btn='<button data-toggle="tooltip" data-placement="top" title="Delete" type="button" class="btn btn-xs btn-danger btn-icon team_delete_onclick" data-id="'.$result[$i]['id'].'" ><i class="fas fa-trash"></i></button>';	
...
//Function for team addedit
if($actionType=="teamAddEdit")
{
...
$title=$app->getPostVar('title');
	...
	if($title!='')
	{
		...
		$obj_model_user = $app->load_model("team");
		...
		if($rs>0)
		{
			$msg="Team Record ".$update_title." Successfully.";
			$msgcode=0;
		 }
		 else
		 {
...
if($actionType=="teamDelete")
{
	$getid=$app->getPostVar('getid');
	
	if($getid!= NULL && $getid>0)
	{
		$obj_change_table = $app->load_model('team');
