 ...
 * @access public
 */
class Evento_Customize_Control_Tabs extends WP_Customize_Control {

	/**
	 * evento_Customize_Control_Tabs constructor.
	 *
	...
	public function render_content() {
		...
		foreach ( $this->tabs as $value => $args ) {
			if ( ! empty( $args['controls'] ) && ( $this->tab_has_controls( $args['controls'] ) ) ) {
				...
				$output .= '</label>';
				$output .= '</div><!-- /.evento-customizer-tab -->';
			}
		}
		$output .= '</div><!-- /.evento-tabs-control -->';
	...
	public function enqueue() {
		...
		wp_enqueue_script( 'evento-tabs-control-script', EVENTO_PLUGIN_URL . 'inc/custom-controls/Tabs/js/script.js', array( 'jquery' ), 'evento', true );
		wp_enqueue_style( 'evento-tabs-control-style', EVENTO_PLUGIN_URL . 'inc/custom-controls/Tabs/css/style.css', null,'evento');

	}

	/**
	 * Enqueue the partials handler script that works synchronously with the evento-tabs-control-script
	 */
	public function partials_helper_script_enqueue() {
		wp_enqueue_script( 'evento-tabs-addon-script', EVENTO_PLUGIN_URL . 'inc/custom-controls/Tabs/js/customizer-addon-script.js', array( 'jquery' ),true );
	}
