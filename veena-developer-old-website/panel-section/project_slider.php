...
include 'sessions_file_project.php';
 ...
 if (isset($_GET["mode"]))
 {
     if ($_GET["mode"] == "delete_item")
     {
         ...
         $page = $_GET["page"];
         $Query = "delete from msp_project_slider where project_sliderid = '$item_id'";
         $ProjectSlider->dbquery($Query);
         
         header("Location: project_slider.php?wb=$wb&action=remove");
         exit();
     }
 }
     ...
     <div class="row wrapper border-bottom page-heading">
       <div class="col-lg-12">
         ...
         <ol class="breadcrumb">
           ...
           <li class="active"> <strong>Slider</strong> </li>
         </ol>
       </div>
     </div>
     ...
     if ($_GET["action"] == "update") {
           ...
           <div id="showpopup"></div>
             <div class="ibox-title">
               <h5>Slider Manegments</h5>
               <div class="ibox-tools" > <a class="btn aqua btn-xs" href="#" onclick="window.location='add_project_slider.php?wb=<?=$SessionWebsiteID;?>'" >Add New Slider</a> </div>
             </div>
             ...
             <div class="ibox-content collapse in">
               <div class="widgets-container">
                   ...
                   <table id="example6" class="display nowrap table  responsive nowrap table-bordered" cellspacing="0" width="100%">
                     ...
                     <?php
                     $ProjectSliderNum = $ProjectSlider->GetProjectSliderByWebsiteNum($SessionWebsiteID);
                     $ProjectSliderRes = $ProjectSlider->GetProjectSliderByWebsiteRes($SessionWebsiteID);
                     ...
                     if ($ProjectSliderNum > 0)
                     {
                       ...
                       ?>
                       <tr>
                         <td><img src="../images/slider_images/<?=$ProjectSliderImages;?>" width="100%"></td>
                         <td><?=$ProjectSliderTitle;?></td>
                         <td><?=$ProjectSliderStatus;?></td>
                         <td class="project-actions">
                         <a class="green btn btn-outline btn-xs" href="edit_project_slider.php?wb=<?=$SessionWebsiteID;?>&ProjectSliderID=<?=$ProjectSliderID;?>" data-toggle="modal" ><i class="fa fa-pencil"></i> Edit </a> 
                         <a class="red btn btn-outline btn-xs" href="#" onClick="javascript:DeleteParentImage('<?=$ProjectSliderID;?>', 'project_slider', '<?=$SessionWebsiteID;?>');"  ><i class="fa fa-trash-o"></i> Delete </a> </td>
                       </tr>
                       <? }
                       } ?>
