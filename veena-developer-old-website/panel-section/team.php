...
include 'sessions_file.php';
  if (isset($_GET["mode"]))
  {
      if ($_GET["mode"] == "delete_item")
      {
          ...
          $page = $_GET["page"];
          $Query = "delete from msp_team where teamid = '$item_id'";
          $Team->dbquery($Query);

          header("Location: team.php?action=remove");
          exit();
      }
  }
      ...
      if ($_GET["action"] == "remove") {
            ...
            <div id="showpopup"></div>
              <div class="ibox-title">
                <h5>Team Manegments</h5>
                <div class="ibox-tools" > <a class="btn aqua btn-xs" href="#" onclick="window.location='add_team.php'" >Create new team</a> </div>
              </div>
                    ...
                    <table id="example6" class="display nowrap table  responsive nowrap table-bordered" cellspacing="0" width="100%">
                      ...
                      if ($TeamNum > 0)
                      {
                        while ($TeamData = $Team->dbfetch($TeamRes))
                        {
                          ...
                          $TeamMobile = $TeamData["msp_mobile"];
                          $TeamEmail = $TeamData["msp_email"];
                          $Teamcategory_id = $TeamData["category_id"];
                          $TeamImages = $TeamData["msp_image"];
                          $TeamStatus = $TeamData["msp_status"];
                        ?>
                        <tr>

                          <td> <img src="../images/team_images/<?=$TeamImages?>" alt="<?=$TeamTitle?>"/></td>
                          <td><?=$TeamTitle;?></td>
                          <td><?=$TeamPosition;?></td>
                          <td><?=$TeamStatus;?></td>
                          <td class="project-actions">
                          <a class="green btn btn-outline btn-xs" href="edit_team.php?TeamID=<?=$TeamID;?>" data-toggle="modal" ><i class="fa fa-pencil"></i> Edit </a> 
                          <a class="red btn btn-outline btn-xs" href="#" onClick="javascript:DeleteItem('<?=$TeamID;?>', 'team', '<?=$page;?>');"  ><i class="fa fa-trash-o"></i> Delete </a> </td>
                        </tr>
                        <? }
                        } ?>
