...
if (isset($_POST["mode"]))
{
    if ($_POST["mode"] == "add")
    {
        ...
        if ($PageInfoNum > 0 )
        {
            $sqlQuery = "UPDATE `$TableName` set `page_title` = '$page_title', `page_keyword` = '$page_keyword' , `page_description` = '$page_description' , `title` = '$title' , `contents` = '$contents' where websiteid= '$SessionWebsiteID'";
        ...
        {
            $add = "INSERT INTO `$TableName` (`websiteid` ,`page_title` , `page_keyword` , `page_description` , `title` , `contents`) VALUES ('$SessionWebsiteID','$page_title', '$page_keyword', '$page_description', '$title', '$contents')";
