...
$mySelectDB=mysqli_select_db($con,'veenabki_demo') or die('could not connect to the database');
  ...
  if (isset($_GET["mode"]))
  {
      if ($_GET["mode"] == "delete_item")
      {
          ...
          $page = $_GET["page"];
          $Query = "delete from msp_slider where sliderid = '$item_id'";
          $Slider->dbquery($Query);
          
          header("Location: slider.php?action=remove");
          exit();
      }
  }
      ...
      if ($_GET["action"] == "remove") {
            ...
            <div id="showpopup"></div>
              <div class="ibox-title">
                ...
                <div class="ibox-tools" > <a class="btn aqua btn-xs" href="#" onclick="window.location='add_slider.php'" >Create new slider</a> </div>
              </div>
              ...
              <div class="ibox-content collapse in">
                <div class="widgets-container">
                    ...
                    <table id="example6" class="display nowrap table  responsive nowrap table-bordered" cellspacing="0" width="100%">
                      ...
                      <?php
                      $qry = mysqli_query($con,"SELECT * FROM `msp_slider` where msp_status='Active' order by sliderid desc ");
                      $SliderNum = mysqli_num_rows($qry);
                      
                      if ($SliderNum > 0)
                      {
                        while ($SliderData = mysqli_fetch_array($qry))
                        {
                        ...
                        ?>
                        <tr>
                          <td><img src="../images/slider_images/<?=$SliderImages;?>" width="100%"></td>
                          <td><?=$SliderNum;?></td>
                          <td><?=$SliderStatus;?></td>
                          <td class="project-actions">
                          <a class="green btn btn-outline btn-xs" href="edit_slider.php?SliderID=<?=$SliderID;?>" data-toggle="modal" ><i class="fa fa-pencil"></i> Edit </a> 
                          <a class="red btn btn-outline btn-xs" href="#" onClick="javascript:DeleteItem('<?=$SliderID;?>', 'slider', '<?=$page;?>');"  ><i class="fa fa-trash-o"></i> Delete </a> </td>
                        </tr>
                        <? }
                        } ?>
