...
include 'sessions_file_project.php';
 ...
 if (isset($_GET["mode"]))
 {
   if ($_GET["mode"] == "delete_item")
   {
    ...
    $page = $_GET["page"];
    $Query = "delete from msp_project_video where project_videoid = '$item_id'";
    $ProjectVideo->dbquery($Query);
    
    header("Location: project_video.php?wb=$wb&action=remove");
    exit();
   }
 }
     ...
     if ($_GET["action"] == "update") {
           ...
           <div id="showpopup"></div>
             <div class="ibox-title">
               <h5>Video Manegments</h5>
               <div class="ibox-tools" > <a class="btn aqua btn-xs" href="#" onclick="window.location='add_project_video.php?wb=<?=$SessionWebsiteID;?>'" >Add New Video</a> </div>
             </div>
             ...
             <div class="ibox-content collapse in">
               <div class="widgets-container">
                   ...
                   <table id="example6" class="display nowrap table  responsive nowrap table-bordered" cellspacing="0" width="100%">
                     ...
                     <?php
                     $ProjectVideoNum = $ProjectVideo->GetProjectVideoByWebsiteNum($SessionWebsiteID);
                     $ProjectVideoRes = $ProjectVideo->GetProjectVideoByWebsiteRes($SessionWebsiteID);
                     ...
                     if ($ProjectVideoNum > 0)
                     {
                       ...
                       ?>
                       <tr>
                         <td><embed src="http://www.youtube.com/v/<?php echo $video_id; ?>&hl=en&fs=1" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" width="100%" height="100%"></td>
                         <td><?=$ProjectVideoTitle;?></td>
                         <td><?=$ProjectVideoStatus;?></td>
                         <td class="project-actions">
                         <a class="green btn btn-outline btn-xs" href="edit_project_video.php?wb=<?=$SessionWebsiteID;?>&ProjectVideoID=<?=$ProjectVideoID;?>" data-toggle="modal" ><i class="fa fa-pencil"></i> Edit </a> 
                         <a class="red btn btn-outline btn-xs" href="#" onClick="javascript:DeleteParentImage('<?=$ProjectVideoID;?>', 'project_video', '<?=$SessionWebsiteID;?>');"  ><i class="fa fa-trash-o"></i> Delete </a> </td>
                       </tr>
                       <? }
                       } ?>
