...
if (isset($_POST["mode"]))
{
    if ($_POST["mode"] == "add")
    {
            ...
            $status=$_POST['status'];
            $date=date('d/m/Y');
            $qry="INSERT INTO `msp_testimonial`
            (`msp_testimonial_id`, `msp_testimonial_text`, `msp_testimonial_author`, `msp_testimonial_desg`, `msp_testimonial_company`, `msp_testimonial_date`, `msp_testimonial_status`) 
            VALUES (null,'$testi','$author','$desg','$comp','$date','$status')";
            $qry1=mysqli_query($con,$qry);
          if($qry1){
            $action='add';
              header("Location: testimonial.php?action=$action");}else{
                   $action='sub';
              header("Location: add_testimonial.php?action=$action");
            ...
            <div class="widgets-container">
              ...
              <input name="old_image" id="old_image" value="<?=$imagename;?>" type="hidden">
                <div class="form-group">
                  ...
                  <textarea class="form-control"  name="testimonial" id="testimonial" value="<?=$testimonial;?>" placeholder="Testimonial Text" rows="5" ></textarea>
                </div>
                ...
                <button type="submit" class="btn aqua m-t-xs bottom15-xs">Add Testimonial</button>
              </form>
            </div>
          </div>
        </div>
