...
$mySelectDB=mysqli_select_db($con,'veenabki_demo') or die('could not connect to the database');
	...
	{
		$sqlQuery = "SELECT eventid from msp_event where msp_status='Active'";
	...
	{
		$sqlQuery = "SELECT eventid from msp_event where msp_status='Active' LIMIT $offset, $limit";
		$sqlRes = $this->dbquery($sqlQuery);
		$sqlTotal = $this->dbnumrow($sqlRes);
		if ($sqlTotal > 0 )
			return $sqlTotal;
	...
	{
		$sqlQuery = "SELECT * from msp_event where msp_status='Active' order by eventid desc";
		$sqlRes = $this->dbquery($sqlQuery);
		return $sqlRes;
	}
	...
	{
		$sqlQuery = "SELECT * from msp_event where msp_status='Active' order by eventid asc LIMIT $offset, $limit";
		$sqlRes = $this->dbquery($sqlQuery);
		return $sqlRes;
	}
